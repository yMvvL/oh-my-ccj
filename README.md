...
| MCP servers | A server configured in `<home>/mcp.json` contributes its tools to this agent, named `mcp__<server>__<tool>` so an approval prompt, a rule and the transcript can all say where a capability came from. Started the first time one of their tools is called — discovery asks each server once and closes it, so a server nobody uses costs nothing — over the server's own stdin/stdout, with a deadline per request, stderr drained, and the process killed when ccj exits. Every call goes through the same approver as `bash`: a server's idea of what it may do is not this program's. See [docs/MCP.md](docs/MCP.md) |
| Undo | Every turn that changes a file can be taken back: `POST /api/undo` (or `/undo` in the REPL, or the composer's Undo button) puts the files of the last turn back as the turn found them — including deleting a file the turn created — and each press goes one turn further back. Snapshots live beside the session (`<id>.checkpoints/<turn>/`), the twenty most recent are kept, a file over 4 MB is skipped rather than copied, and a file outside the session's directory is not this conversation's to undo. This is the other half of approval: approval answers "may it run", undo answers "may it be taken back", and only the second one is what lets somebody leave the guard on. |
| Prompt caching | Anthropic requests carry three `cache_control` breakpoints — the system prompt, the tool set, and the end of the conversation — so turn *n+1* pays a cache read for the prefix turn *n* already sent instead of writing it again. Nothing else about the request changes, and the OpenAI-shaped path needs none: its prefix caching is automatic and its hits already show up in the panel's cache rate. |
| Automatic compaction | The same `maxContextTokens` that trims a request now also compacts the *conversation* when it grows past it: between turns, never inside one, and only when the summary is genuinely smaller. The alternative is what the projection does on its own — eliding tool output and dropping whole exchanges, with a notice saying how many went and no summary saying what they were. No budget configured means no automatic compaction; a refusal is remembered at double the size, because measured live a reasoning model can write a summary larger than the exchanges it replaces, and a refusal that retried every few turns would be a paid call that always fails. |
