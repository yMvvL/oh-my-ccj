...
- 图片是单向的，而且隔了一层：图像由视觉模型描述，*描述*加入对话。主模型从不接收图像，所以「这个错误说的
  ...
  或者 `read` 那个文件，因为它被告诉去看那里。每个回合一张图片，最多 8 MB，第二次上传会开始第二个回合。
