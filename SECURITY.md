...
**没有**沙箱：它就是带你的 PATH、你的 `$HOME` 和你的凭据的那个 shell——`bash` 工具与编辑后检查都用配置
里那个（默认 `/bin/bash -lc`，见 `shell` / `--shell` / `CCJ_SHELL`），你能读写的东西它都能读写。
审批是它动手之前唯一的防线，而 `--yolo`（或自动批准）按设计会把这层防线拿掉。请把正在运行的 ccj 完全
当成一个开着的终端来对待。

审批现在有五种回答而不是两种——拒绝、允许一次、本会话内允许、从现在起允许、**从现在起拒绝**——因为另
一种做法是一个开关，一按就把整场会话的防线关掉，而人们会关掉的防线就不是防线。最后这两种是一对：一个写
下回答「可以」的规则，一个写下回答「不行」的规则，写进同一份文件，主语也一模一样，所以被同一条命令反复
问到的人有了一条比关掉整道防线更窄的出路。每一种各做什么写在下面的**审批规则**一节，同样写在那里的还有
真正要紧的部分：自动回答的宽度，只等于当初授予它的那件事。

## 防住了什么

- **默认绝不绑通配地址。** 服务器绑 loopback，加上这台机器有 tailnet 时的 tailnet 地址——绝不是
  `0.0.0.0`。通配会把端口放到这台机器所在的每一个网络上。显式要求通配地址是允许的（有时你就是要从
  同一个 wifi 上的平板连过来），但那会打印一行警告说明代价，并给出更窄的那条路。
- **每个非 loopback 地址都有 token。** 生成出来的是 32 个字节的 `SecureRandom`——256 位，写成 64 个
  十六进制字符——存在 `~/.oh-my-ccj/web-token`，权限 `0600`，首次需要时生成。loopback 不需要：为了用
  你自己的命令行还要输个密码，保护不了任何东西。生成代替向人要，因为一个人编出来的值会是 256 位设计里
  最弱的一环；而这里也没有需要查重的地方——撞上同一个值要大约 2^128 次生成。需要的另一件事是「一次
  安装只有一个 token」：两个进程同时启动时（一个重启、第二个工作区），创建是独占的，输的那一方读赢家
  写下的值。自己设的 token 短于 16 个字符时启动会警告一次——它是这台机器上唯一的门禁，而短到可以猜的
  秘密比没有秘密更糟，因为它看起来像是锁着的。
...
- **图片上传是唯一接受攻击者构造字节的端点，而它有三道边界。** 类型从魔数读，不是从文件名或
  ...
  攻击。**缩略图端点读的是同一批字节，而它有一道自己的边界：** `GET /api/attachment/thumb?name=…` 的
  名字是调用方给的，所以那里检查的是解析结果而不是拼写——名字解析之后仍必须落在这条会话自己的附件目录里，
  `../`、绝对路径和指向别处的符号链接都当没有这个文件。它服务的是这条会话里**已经存在**的图片，不做任何
  新的读取。
- **`fetch` 是唯一请求会到达第三方的工具，因此它被相应地圈起来。** 在拨号之前先检查 scheme（只允许
  ...
  *在读取过程中*封顶的。它像其它一切一样请求审批，而 URL 就是规则的主语：
  ...
  `=`、`:`）之后，因为 `https://docs.example.com*` 也会覆盖 `https://docs.example.com.evil/`。可选参数
  `as: "text"` 只是把已经读回来的那份 HTML 剥成文本（并且声明这是一次猜测），它发生在有界读取*之后*，
  改的是交出去的文字，不是这一次请求的边界。它不做的事：没有 JavaScript、没有 cookie、没有凭据、没有
  POST——登录之后的页面抓到的是陌生人看到的样子，模型在工具描述里就被这么告知，而不是从一个空回答里
  自己发现。
...
- **配置文件里声明的检查不经提示就运行。** `"checks"` 指名一些在编辑之后运行的命令——一次编译、一次类型
  ...
  提示地运行——代价是对那次写入的一次审批，这和项目的 `CCJ.md` 已有的暴露面相同。这份放宽是**定下来
  的**，不是待办：让检查走审批的代价是每次编辑弹一次提示，也就是把一个「编辑之后就告诉你结果」的功能变成
  一个「先点两次再说」的功能；走规则那条路也不行，因为一条允许规则会让它**永远**不问，那和一个写明的放宽
  是同一件事，只是多了一层假装。它没有让任何新的东西变成可能：那条命令是用户自己写进配置文件里的，这和
  把它写进 `CCJ.md`、或者直接在终端里敲出来是同一件事；而它同样没有变宽——只运行配置里那条命名的检查
  命令，在会话的工作目录里，仅此而已。`tool/PostEditCheck.java` 的 javadoc 里写着同一段理由。

## 审批规则

一次审批是一个有五种回答的问题，其中三种可以被记住：
...
| 从现在起拒绝 | 一条 **deny** 规则，主语与「从现在起允许」完全相同；本次调用被拒绝，下一次同样的调用由规则挡下，不再问人 | 同一个文件，`deny` 列表，`0600` |
...
- **拒绝永远优先**，包括优先于本会话允许、也优先于写进文件的允许规则：按一下「本会话内允许」不能
  撤销一条禁止某命令的规则。而「从现在起拒绝」写下的那条规则就在 `deny` 列表里，和手写进去的那条是
  同一种东西——它压过一切放行，只是因为它回答的是「不行」，而不是因为它是谁写的。
...
- **什么都不审计，也不做速率限制。** 没有账号，除了会话文件没有日志，也没有办法在不轮换 token 文件的情况下
  吊销一台设备。
- **不要用来暴露给公网。** tailnet 是一组你管理的设备；VPS、端口转发或咖啡店网络不是。**哪些路在
  信任模型里、哪些不在，[docs/WEBUI.md](docs/WEBUI.md) 的「从别的设备访问」列了全部六条**：loopback、
  mesh VPN、SSH 端口转发是你自己的设备集合；局域网、隧道和公网反向代理不是。这条限制没有变，那一节也
  不是对它的一次放宽——token 是一个不带速率限制、不带审计的 bearer secret，它过滤来者，并不能让这个
  服务可以安全地对外发布。
