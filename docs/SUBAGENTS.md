...
默认是**跟随主对话**：一次委派用的是主对话那把钥匙、那个端点，和那个模型。想让某个角色比主对话更
省（或更强）时，按角色点名，两个地方写的是同一件事：

```
--subagent-model explore=cheap-model --subagent-reasoning explore=low        （命令行，可重复给出）
CCJ_SUBAGENT_EXPLORE_MODEL=cheap-model                                       （环境变量）
```

```json
{ "subAgents": { "explore": { "model": "cheap-model", "reasoning": "low" } } }
```

`model` 和 `reasoning` 两项都可以只点一个，没点的那个仍然跟随主对话；表里没出现的角色全都跟随主
对话。档位是 `low`、`high`、`max`。

为什么默认是跟随，而不是每个角色写一份默认值：只有一个地方说「这次运行的模型是什么」，用户想换
模型时就换那一个地方。钉住是一次收窄——被点名的角色换了模型和档位这两栏，温度、token 上限、上下文
预算和审批者一个字都不变，因为一次委派不是另一个提供方，也没有第二份登录。同理，一次委派花掉的
token 仍然记在启动它的那个对话的账上；它花了多少会跟着报告回来，所以那笔账看得见，而不只是被算进去。
...
USAGE: 输入 / 输出 / 缓存 的 token 数，加上模型回合数
FILES:
...
`USAGE` 那一行说的是**这次运行自己花了多少**。转录只会说某个任务跑过，不会说它花了多少，而
子代理的存在本来就是为了让一次长长的阅读留下很短的一段话——这两件事一起，会让一次昂贵的委派
看起来和一次便宜的一样。读取的过程仍然不进任何一行；账单进来。提供方一个数字都没报时，这一行
就不写：凭空写一行零，比没有这一行更糟。角色被钉到主对话以外的某个模型时，这一行也会点名那个
模型。
