...
## 2026-09-21 — README 再砍一刀：269 → 102 行

上一刀把**手册**搬走了，这一刀砍的是剩下那半里的东西：一张 30 行的能力表（单元格长到几百字）和一整份限制
清单。理由不是「太多了」，是**它把不该由这个 README 讲的事也讲了**：八个工具、会话恢复、压缩、MCP、子代理、
工作区、提供方——这些别的代理都有，写在门面上既不构成差异，也让人找不到真正不一样的那几件。

真正不一样的是四件，现在各占一段：**图片从手机进来**（独立视觉模型写描述，主模型从不看图像）、**同一个 jar
也是网页控制台**（手机与笔记本说同一份本地文件，无账号无云）、**零密钥 `--demo`**（别家都要一把密钥或自备
端点）、**审批是唯一防线而且它可以被记住**（五个答案、规则文件、子代理问同一个人）。第五段是性格：
**每个主张都带着它的测量结果**。

搬走的（原文一字未改，进 `docs/USAGE.md`）：能力表整张 → 「全部能力，一条一条」；限制清单整份 →
「限制（完整清单）」。README 那节留六条**必须知道**的（无沙箱、图片是单向的、没有步数上限、每会话单回合、
同一工作区里多个对话没有先后顺序、会话文件无上限增长），末尾加一句指向完整清单。`docs/BOOTSTRAP.md` 与
`docs/VISION.md` 里两处「见 README 的某节」跟着改到新家。

## 2026-09-21 — README 变成门面，以及一条只在套件里犯病的测试

**README 593 行 → 269 行。** 长的不是「它是什么」那一半，而是手册那一半：用法（`--help` 的逐字副本）、
配置与环境变量、工具表、工作区、架构、开发与测试数表——约 180 行纯参考资料，另有约 150 行与 `docs/` 里
已有的文档**说同一件事**，而「同一件事有两个地方说」正是这个仓库自己的约定禁止的。现在它们各归其位：
**新的 `docs/USAGE.md`**（用法、配置、工具、工作区、自带运行时的包）、`docs/WEBUI.md`（手机与 Tailscale
那一节并进已有的「从别的设备访问」）、`docs/ROUTER.md`（提供方、端点与密钥）、`docs/ARCHITECTURE.md`
（目录职责那张表）、`CONTRIBUTING.md`（怎么跑测试与按包的测试数）。README 留下门面：它是什么、为什么、
两分钟跑起来、它刻意不做什么、限制，末尾一张指向 `docs/` 的清单。`DocsConsistencyTest` 跟着搬到新家——
它守的那条主张没有变。

顺带抓到两处**真的矛盾**（都是搬的时候逐条核对出来的）：`docs/WEBUI.md` 写着「只有两种协议存在」——而
...
自己那一遍只跑它。不需要把整棵套件降级成「每个类一个 JVM」（那样 CI 会慢上一倍），而隔离之后连跑两遍
全量都过，它自己仍是 1 秒。

## 2026-09-21 — 把两个巨型文件切开

`web/AgentHub.java` 2811 行、`web/WebApiTest.java` 3942 行。它们都不算 bug，但「一个下午能读完」是这个项目
的卖点之一，而这两个文件正在把那个卖点吃掉——而且它们是**用一次读不完**的那种大：找 `applyRules` 要滚过两千
行，改一条审批断言要先在一片 126 个用例里定位。

**主代码**：`AgentHub` 抽出三块，各自独立成类——`EventStream`（SSE 扇出：订阅者、重放环、事件编号与
`publish`）、`ApprovalDesk`（审批台：待批列表、五个答案的解析、提示的发布、看门狗与 `gate()` 的组装）、
`SettingsFiles`（规则与检查这两个文件的读改写，含「写坏了就还原」那条路）。2811 → 2428 行，新添 78 + 265 + 230
行。这不是「拆成微服务」那种拆法：三块本来就不共享 AgentHub 的状态，各自持有自己那份，需要回调时注入回调，
而不是把 `this` 传出去。

**测试**：`WebApiTest` 按主题切成九个类共用一个夹具 `WebHarness`（610 行，非 `*Test` 故 surefire 不当它测试
跑——与 `ScriptedProvider`、`FakeServer` 同一先例）。**126 个用例一个不少、方法体一字未改**，切成：页面与
SSE 流 10、审批握手 10、会话并发 13、会话与工作区 19、账本与压缩 16、设置表单 10、提供方目录 21、图片与
视觉 13、跨源与 token 与 `Host` 14。最大的测试类从 3942 行降到 562 行。

**行为不变是怎么保证的**：只做机械搬运（方法体、javadoc、注释原样），唯一的可见性改动是把几处 `private`
放宽到包内——新类要用；唯一严格讲不等价的是一处 volatile 字段在门面里取值早了一瞬，而那是一个不可能发生的
竞态（那个字段只在服务器起来之前被设置过一次）。证据是安全网本身：**743 个测试全绿**，与切分前逐字相同。

顺带修掉几处指向旧文件的文档（README 里那条 `-Dtest=WebApiTest` 命令、`ARCHITECTURE` 的类表、`RESUME` 与
`VISION` 里的点名）。
