...
消息的形状定了下来：`[picture <name>] <description>`，后面跟一行指明文件。够不着、配置错误或未配置的
视觉模型会拒绝这张图片并说明要修什么（`AgentException` 指明端点和状态，或者一个 `409` 指明 `vision` 块和
那些标志），没有回合开始，什么都没写。

**用起来之后的修正：上传只描述，不发送。** 按上面那样实现出来的东西，在真机上用起来是错的，而错在
一个设计里没有问过的问题上：*谁来决定拿这张图片做什么？* 上传会立刻开启一个回合，于是模型先收到一段
描述、自己决定去看它、然后**在用户说出想让它做什么之前就已经开始干活**——用户以为自己在「传一张待会
要用到的图」，拿到的却是一个自作主张的回合。现在描述挂在这条会话上等着，用户的下一句话和它拼成一条
消息（描述在前，要求在后），所以模型一次收到「图片里有什么」和「照这个做」两件事，而这两个句子的顺序
是刻意的：模型读到末尾时，最后一件读到的事就是要它做的事。随之而来的两个变化：一张图可以在一个回合
正在跑的时候上传（描述不是那个回合的工作，结果就停在那里等着，而旧的「先中止它」把一次上传变成了一条
死路），第二次上传取代还在等着的那一张而不是排成第二个回合（一次只有一张会跟着消息走）。

同一轮真机体验还改了另一半：**相机**。原来的按钮只有 `accept="image/*"`，在手机上调图库——拍一张的
路根本不存在，而这正是这个功能最主要的用法。`capture` 是按钮级的属性，一个按钮没法同时表示「拍一张」
和「选一张」，所以按钮现在先问来源，再打开对应的那个文件输入。
...
- 「一张图片从手机到达」的 **tailnet** 那一半，是拿这台机器自己的 tailnet 地址上的真实服务器验证的，用一个从 `http://100.64.0.1:6767/` 加载的真实浏览器页面：消息发过去了，选择器打开了，上传到达了端点。那次检查发现了一个与图片毫无关系的缺陷——跨源护栏只接受回环来源，所以来自 tailnet 页面的*每一个*状态更改请求都被拒绝；修复及其推理在 [SECURITY.md](../SECURITY.md) 和 [WEBUI.md](WEBUI.md)。
...
- **一条消息里多张图片？** 仍然一次一张，但机制换了：第二次上传**取代**还在等着的那一张，而
  `replaced` 说的是它取代了谁。排队成第二个回合在只有一张图片会跟着消息走时毫无意义，而一次静默的
  替换至少是被说出来的。
