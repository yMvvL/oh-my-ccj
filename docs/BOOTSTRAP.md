...
- `ccj.cmd` 是这些规则的 Windows 版本：`%~dp0` 定位仓库根，退出码 75 时同样重新启动，`-p` 同样不重发。差别只有一处，在读这一节时要记住：它**只判断 `target\ccj.jar` 在不在，不判断源码新旧**——`find -newer` 在 CMD 里没有对应物。所以要它重建，先删掉那个 jar。它没有人在真的 Windows 上跑过，见 README 的「运行环境」。
