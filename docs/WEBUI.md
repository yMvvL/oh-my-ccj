...
| `GET` | `/api/status` | 提供方、模型、base URL、cwd（以及 `-C` 是否覆盖了它）、会话、工具、当前屏幕上那个会话的 `busy`，以及 `running`——此刻正在工作的其它会话的 id；还有这条会话的花费上限 `maxTotalTokens`，没设上限时是 `null` |
| `GET` | `/api/events` | 循环上报的一切组成的 SSE 流 |
| `POST` | `/api/message` | `{"text": "...", "picture": "photo.jpg"}`——开始一个回合，或者**把它排到正在跑的那个回合后面**：两种情况都返回 `202 {"accepted": true, "queued": true\|false}`。只有队列满（16）、没有配置模型、或这条会话**已越过花费上限**时才返回 `409`——最后一种带着已用多少、上限多少、以及三种改法（flag、配置文件、环境变量），而且判定在回合开始**之前**，和终端里是同一条规矩。队列是按对话分的，所以另一个对话仍然立刻就能开始 |
...
| `GET` | `/api/attachment/thumb?name=...` | 这条会话附件目录里某张图片的小图，供转录里的历史消息和待发送的缩略条使用。只服务**这条会话自己的附件目录**：名字解析之后仍必须落在那个目录里，`../`、绝对路径和指向别处的符号链接都当没有这个文件（`404`）。最长边 96 像素的 PNG，每次请求现画、磁盘上不留缓存；解不出来的格式（WebP 是一例）**原样交回字节**，所以画不出小图不是一次错误。响应带 `Cache-Control: no-cache`，因为同一个名字被取代之后可能是另一张图 |
| `GET` | `/api/undo` | **预览**：回答「按下去会动什么」——`200 {"files":[{"path":"notes.txt","action":"restore"\|"delete"}],"turns":n}`，`turns` **含这一次**在内，没有可退回的回合时 `files` 为空、`turns` 为 0。它不建目录、不写文件、不删回合，什么都不改；清单里只有下一次退回真会动的那些文件。回合正在跑时返回 `409` |
| `POST` | `/api/undo` | **执行**：把上一个回合改动过的文件放回去（该回合新建的删掉），`200 {"restored": n, "files": [...], "remaining": m}`，并在转录里留一条通知。回合正在跑时返回 `409`——在它底下做退回，会把模型正想着的文件还原回去 |
...
`GET /api/config` 还会报告用于描述图片的模型，并接受把它传回来：
...
门禁和布局。**Tailscale 不是必需的**——它只是最省事的那条路，因为不用把端口暴露给别人、不用管
证书、地址还固定。下面这张表是全部几条路，以及每条路各自要注意什么。

| 怎么连 | 命令 | 手机／另一台设备打开 | 注意 |
|---|---|---|---|
| **tailnet（Tailscale）** | 直接 `ccj` | 打印出来的第二个 URL，带 `?token=…` | 最省事：装完之后什么都不用传。本机在 tailnet 上时默认就顺带监听它 |
| **别的 mesh VPN**（ZeroTier 等） | `ccj --host 10.147.x.y` | `http://10.147.x.y:6767/?token=…` | 等价，但**自动发现不认**（它只接受 `100.64.0.0/10`），地址得自己填 |
| **同一个局域网** | `ccj --host 0.0.0.0` | `http://192.168.x.y:6767/?token=…` | token 明文过网络，只在可信网络这么干；通配地址会收到一行警告 |
| **SSH 端口转发** | `ssh -N -L 6767:127.0.0.1:6767 me@host` | 浏览器开 `http://127.0.0.1:6767` | **最安全**：连接是回环，连 token 都不需要；适合另一台电脑，手机要有 SSH 客户端 |
| **隧道**（cloudflared / ngrok / frp） | `cloudflared tunnel --url http://127.0.0.1:6767` | 隧道给出的 HTTPS 域名 | **必须保留 `Host`**，否则每次改动状态的请求都 `403`；而且这等于把 shell 放上公网 |
| **VPS + 反向代理** | `proxy_set_header Host $host;` + TLS | 你自己的域名 | 护栏能过、TLS 由 nginx 给、token 照样要带 |

**它自己不提供 HTTPS**（只有明文的 `HttpServer`），所以 TLS 属于隧道或反向代理那一层。token
的 cookie 也因此不带 `Secure`：明文 HTTP 下它仍然工作，这在局域网和 tailnet 上是想要的。

**判断一条路走不走得通**，只要在这个页面上发一条消息。跨源护栏要求 `Origin` 的主机是回环地址
**或等于这次请求所寻址的 `Host`**，所以一个改写了 `Host` 的反向代理会得到
「跨源（cross-origin）请求被拒绝」，错误消息里带着那个 `Origin`——改配置即可，不用改代码。

**哪几条在信任模型里。** 回环、mesh VPN／tailnet、SSH 端口转发都是你自己的设备集合；局域网、
隧道、公网反代不是。见 [SECURITY.md](../SECURITY.md) 里「不要用来暴露给公网」那一条——它没有
变，而上面那几行不是对它的一次放宽：token 是一个不带速率限制、不带审计的 bearer secret。
...
oh-my-ccj 0.1.0 — web UI：http://127.0.0.1:6767/
                          也可通过 http://100.64.0.1:6767/?token=… 访问
```

`--host tailscale` 表示*只要*那个 tailnet 地址（给那些不希望别人访问其回环地址的机器），而
`--host <addr>` 仍然表示恰好一个地址。默认永远不使用通配地址：监听 `0.0.0.0` 会把端口暴露在咖啡馆
的 wifi、办公室局域网和 docker 网桥上，而「给地址起名」的意义就在于：能连上这个端口的设备集合，
是用户自己选出来的集合。显式要求通配地址是允许的——那时启动会打印一行警告，说明这样做的代价，
并给出更窄的那条路。tailnet 地址通过 `tailscale0` 网卡找到；如果 tailnet 挂在名字不同的
...
权限模式 `0600`。**生成而非向人要**：32 个字节的 `SecureRandom`，也就是 256 位，写成 64 个十六
...
图片有三个入口，而它们**最后都调用同一个 `sendPhoto`**——列在这里是因为「入口只有一条」正是这个
功能上一次坏掉的地方：上传曾经自己开启一个回合，而描述本该等着和用户的下一句话一起发；多一条图片
路径，就是多一个会那么干的地方。

| 手势 | 它怎么走 |
|---|---|
| **图片**按钮 → **相机** / **相册** | 先问来源，再打开对应的那个文件输入。两个输入是必需的：`capture` 是按钮级的属性，一个按钮没法同时表示「拍一张」和「从图库里选一张」，而图库必须仍然可达 |
| **粘贴**（Ctrl+V / ⌘V） | 剪贴板里是图片就上传它，并且接管这次粘贴；是文本就什么都不做，让它照常落进输入框 |
| **拖放** | 拖进窗口时输入框亮起来，放下第一张图片就上传；放下的不是图片时说一句话，而不是静默地什么都不做 |

一次只上传一张（一次只有一张会跟着消息走），而拖进来好几张时取其中第一张图片。

进入对话的是一段由独立视觉模型写出的*描述*——主模型从不接收图像。**描述不开启回合**：它挂在
这条会话上等着，输入框上方出现一条缩略条，你的下一句话把它一起带走。完整设计（包括为什么是这个
形状）在 [VISION.md](VISION.md) 里；这里讲的是页面和这三个端点（上传、丢弃、缩略图）做了什么。
...
同一段文本在页面上的读法是另一次摆放：它以 `[picture <名>]` 开头时渲染成一条**图片气泡**——缩略图、
文件名、一行「图片 · 由视觉模型描述」，一段**可展开**的描述（`<details>`，摘要写着描述占多少字符），
然后才是用户自己说的那句话。会话文件里那段文本**一个字都没改**：重放、压缩和恢复都靠它，渲染时加
上去的那些节点只存在于这一次显示里；解析不出来（旧格式、人手打的消息）就原样当文本显示，而不是猜。

缩略图由服务器现画：`GET /api/attachment/thumb?name=<名>`，最长边 96 像素的 PNG，磁盘上不留第二份
副本，所以也没有一份会过期、需要清理、或者与原件对不上的缓存。它只服务**这条会话自己的附件目录**
——名字解析之后仍必须落在那个目录里，`../`、绝对路径和指向别处的符号链接都当没有这个文件——而且不
只服务待发送的那一张：历史消息里的图片也要画得出小图，否则刷新或换一台设备之后，一条图片消息就只
剩一个文件名。**画不出小图不是错误**：字节是不是那四种图像之一由服务器说了算，解码交给 JDK，后者
解不了的格式（WebP 是一例）就把原字节交回去——页面不需要兜底，它要么拿到一张 PNG，要么拿到一张
能直接显示的原图。
...
`http://100.64.0.1:6767/` 打开的页面会把那个地址作为 `Origin` 发出去，它既不是回环，也无法
...
| `user` | `text` | 回合的输入，回显出来，好让转录以服务器为准。以 `[picture …]` 开头的那种输入由页面摆成图片气泡，但 `text` 本身和磁盘上的一致，一个字都没改 |
...
| `approval-closed` | `id`, `allow`, `answer` | 已作答（由浏览器或超时）。`answer` 是 `deny`/`once`/`session`/`always`/`never` 里的哪一个，所以五个回答在别处被应答之后，卡片上读到的仍然是它自己的那句话，而不是从一个布尔值去猜；超时那一份带着 `allow: false` 和 `reason: "timeout"` |
...
设了**花费上限**（`--max-total-tokens`、`CCJ_MAX_TOTAL_TOKENS`，或 `config.json` 里的
`"maxTotalTokens"`）时，面板多一行**花费上限：已用 / 上限**——分子是这条会话累计的输入加输出
token，分母是上限。只在服务器报了这个上限时才出现：一行「不设上限」除了占地方没有别的作用。越过
上限之后，`POST /api/message` 以 `409` 拒绝新回合，而不是把正在跑的那个回合砍掉。
...
上面那些用例都只跑渲染器的一小块，所以这条链路的最后一环在真的浏览器里：`src/test/js/browser-smoke.mjs`
起一个 headless Chrome，用 CDP 驱动发布出去的页面走完一个回合——等 `/api/status` 渲染完、往
`#input` 里敲一句话、真的按一下 `#send`、然后等助手的回答出现在转录里。`WebBrowserSmokeTest`
在 `mvn test` 里跑它；这台机器上找不到能用的浏览器（`CCJ_CHROME` 可以指定一个），或者 node 没有
全局 `WebSocket`（需要 22.4 或更新，说 CDP 靠它）时，它报告为 *skipped* 而不是通过。
...
`POST /api/undo` 把上一个回合改动过的文件放回去——该回合新建的删掉，原来就有的放回原样——每按一次
再往前退回一个回合。CLI 的 `/undo` 就是这样，一次到位；页面上它是**两步**，因为它会覆盖磁盘上的
文件，而一个会覆盖文件的按钮值得先说出它要覆盖什么：

1. **第一次按**：向 `GET /api/undo` 要一份**预览**。这个方法什么都不改——不建目录、不写文件、不删
   回合——它只回答下一次会动哪些文件、还有几个回合可退（`turns` **含这一次**）。有可退回的回合就
   把按钮**武装**起来：列出将恢复的文件名（超过三个就写「等 N 个文件」，整句是「将把 … 放回上一回合
   之前的样子」），等「确认退回」或「取消」，按 `Esc` 也取消。没有可退回的回合时说一声「没有可退回的
   改动」，不武装一个什么都不会做的确认——预览这一步同时也回答了「有没有可退的」。
2. **第二次按**（确认）：`POST /api/undo` 真的动磁盘，服务器随后用一条通知说明恢复了什么；转录里的
   说明就在那里，这一侧只报告拒绝。

回合正在跑时两个方法都返回 `409`：预览不该在一个正在改文件的回合底下承诺一份很快就不成立的清单，
执行更不该——那会把模型正想着的文件还原回去。清单里的路径是相对会话的工作目录给出的，而清单里只有
下一次退回真会动的那些文件。

## 审批

一条提示把工具、命令和路径作为自己的字段带上，还有五种回答，页面把它们画成五个按钮：
**拒绝**、**只允许这一次**、**本会话都允许**、**始终允许**、**以后都拒绝**。最后两个是一对，
也是这一节里唯一会把决定写到磁盘上的一对：**始终允许**写下一条回答「可以」的 allow 规则，
**以后都拒绝**写下一条回答「不行」的 deny 规则，写进同一个 `<home>/approvals.json`，主语
一模一样。所以「以后都拒绝」这一次拒绝本次调用，**下一次同样的调用由那条规则挡下，不再问人**——
这让被一个反复出现的提示磨到的人有了一条出路，而不是只有把整场会话的审批关掉。提示里的字段是
`{"id", "title", "detail", "tool", "command", "path"}`。
按钮上的文字就是服务器据以行动的文字（`{"id": "…", "answer": "deny|once|session|always|never"}`），
而两个更旧的布尔字段仍然被理解：`allow: false` 是拒绝；`allow: true`、`remember: true`
现在表示*本会话内的这一个请求*，而不是它过去会开启的整个会话自动批准。字符串那几个取值也有旧名字
（`allow`/`yes` 等于 `once`，`no` 等于 `deny`），而一个不认识的取值是 `400`，不是一次沉默的默认。

回来的结果在 `approval-closed` 事件里以 `answer` 出现，所以在另一个标签页里作答的卡片在这里
读到的是一样的内容——「本会话内已允许」，或者对 `never` 来说是「已拒绝，并写下一条拒绝规则」
——而不是从一个布尔值去猜。每一个不需要询问就给出的回答，
都是转录里的一条 `notice`：`规则批准 — bash: mvn -q -o test`，或者
`规则拒绝 — bash: rm -rf build，规则见 …/approvals.json`。
