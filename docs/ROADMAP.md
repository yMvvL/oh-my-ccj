...
| 最近落地 | **中文**：README、`docs/`、每一行注释与 javadoc、每一条错误与提示、整个网页 UI，以及**图片流程的修正**——上传不再自己开启回合，描述等着和用户的下一句话拼成一条消息，而按钮先问相机还是相册（两者都是在真机上用出来之后改的，见 [VISION.md](VISION.md)）。在那之前：Phase 2.5——围绕循环的循环：编辑之后自动运行的检查、审批规则、排队消息、多块编辑、`fetch`、MCP 服务器、带自动压缩的提示缓存，以及 undo。每一项都在 [CHANGELOG.md](CHANGELOG.md) 里，带着它测量到了什么。在那之前：图片： 每个回合一张图，由一个**独立**的视觉模型描述，其端点、密钥和模型各自配置；描述作为普通文本加入对话，文件存放在会话旁边。主模型从不接收图像，所以两种线路格式、`Message` 和压缩都没有改动。见 [VISION.md](VISION.md) |
| 最近移除 | 子代理暂存目录（`<cwd>/.ccj-work`），以及随之移除的 promote 步骤。审批取代了它——见 [SUBAGENTS.md](SUBAGENTS.md) |

## Phase 0 —— 让主张可核验 —— `done`，留两处尾巴
...
这份清单曾经有三处尾巴。自写下这段文字以来已经关掉了第一处：README 和工作流都不再写明测试数量，工作流的注释说明了原因——这个数字每加一个测试就会漂移，而徽章才是活的答案。剩下两处，每一处都不大，而且都是这一阶段本就是为了消除的那类缺陷。
...
- **3.0 让陌生人试得起来** —— `todo`。README 顶部一段英文摘要（全文仍然是中文，界面也是中文单语——所以
  这一段摘要就是英文读者唯一的门，见 2.6.11）；一段终端录像或 GIF 加一张网页截图；把 `100.72.92.41`（这台机器的 tailnet
  地址，出现在六个文件里）换成占位符；以及一句「这是我自己用的工具，不是产品，我不承诺支持」——把预期放在
  明面上，issue 会少一半。
  *验收：* 一个从没听过这个项目的人，在第一屏看懂它是什么，装得上，并在两分钟内跑出一个真回合。
- **3.1 发布** —— `todo`。带版本号的标签、说明改了什么、代价是什么的发布说明，以及一句声明这个 jar 是为哪个 JDK 构建的。
- **3.2 打包** —— `todo`。fat jar 是今天的方式，而 native image 已经在「不打算做」里排除掉了，所以这里要决定的是另一件事：要不要一个 `jpackage` 做出来的安装包（带启动器、带运行时的镜像），还是就让 jar 加一段两行的说明。
