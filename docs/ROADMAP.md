...
| 2.6.2 | **转录里的图片显示成一条缩略条**，可展开看完整描述。`[picture <name>] <description>` 那段文本**逐字留在会话文件里**，只换渲染 | 今天你发一张图，看到的是一千多字符的英文描述被当成「你说的话」整段塞在气泡里 | 页面上是一条缩略条加一个可展开的描述，而会话文件、重放、压缩、恢复拿到的仍然是同一段文本 | `todo` |
| 2.6.3 | **审批的第五个答案：「以后都拒绝」。** 今天有 `ALLOW_ALWAYS`，却只有一次性的 `DENY`；`DENY_BY_RULE` 只能手写文件 | 四个答案不对称——能永远允许，不能永远拒绝，而后者正是被一个反复出现的提示烦到的人想要的 | 提示上有第五个按钮，它写出的 `deny` 规则挡住同一条命令的下一次调用（真机验：先拒绝，再调用，被规则挡下并说明是规则挡的） | `todo` |
| 2.6.4 | **undo 之前先说会改哪些文件。** `CheckpointStore.undoableTurns` 已经在，缺的只是把它说出来 | 「按下去再发现」不如「先说会改什么」，而退回是唯一一个会覆盖磁盘的按钮 | 按退回之前先看到「将恢复 N 个文件：…」，确认之后才动文件；没有可退回的回合时按钮直接说明 | `todo` |
| 2.6.5 | **设置面板里的审批规则与 `checks`。** 两者今天都只能手编文件（`approvals.json`、配置文件的 `checks`） | 规则是「别老点确认」最关键的机制，而它现在的门槛是「在这台机器上编辑一个 JSON 文件」——手机做不到 | 面板里能增删改，写出来的文件与手写的完全一样，并且下一次提示立刻用上 | `todo` |
| 2.6.6 | **服务端缩略图**（`GET /api/attachment/thumb`） | 刷新页面或换一个设备，只剩下文件名和描述开头——手机上传后切回来，认不出是哪一张 | 换设备打开同一条会话，待发送的图片有缩略图，而不是只有一个名字 | `todo` |
| 2.6.7 | **花费上限。** 今天有上下文预算（`maxContextTokens`），没有「这次最多花多少 token 就停」 | 让它跑长任务时，你真正想知道的是这个数，而不是上下文还剩多少 | 达到上限的那个回合结束时说明花了多少、为什么停；是否继续是用户的决定。**先决定按会话还是按本次运行** | `todo` |
| 2.6.8 | **`fetch` 的显式取文本模式。** 今天响应体原样返回，理由是「剥标签是猜测而不是事实」 | 抓一个网页，token 大半花在标签上；而剥标签可以是**调用方显式要求**的一件事，而不是工具的默认猜测 | 调用方显式要求时才剥，输出里说明自己剥过；不要求时行为一字不变 | `todo` |
| 2.6.9 | **联网搜索走 MCP。** 文档里给一个能照抄的搜索服务器配置示例，核心不写死任何第三方搜索 API | 主流代理都有搜索，而这里既没有、也没有说清楚怎么加；但核心加搜索要一把第三方密钥，与服务端搜索绕过审批的取舍 | README 与 [MCP.md](MCP.md) 里有一个可复制的例子，配一条规则就能用，示例里的包名是核过当下可用的 | `todo` |
| 2.6.10 | **编辑后检查的豁免，做出决定。** 2.5.1 那次写明的放宽，到今天仍然是放宽——`PostEditCheck` 里没有审批者 | 一个悬着的「以后再说」比一个决定贵；而且现在已经有三处文档在描述它 | 一份写下的决定：保持为放宽，或者变成一条普通的允许规则。无论哪个，SECURITY.md、`PostEditCheck` 的注释和这里说的是同一件事 | `todo` |
